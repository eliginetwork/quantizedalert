"""QuantizedAlert front-door CLI.

Every subcommand executes the real pipeline (qlib asset + DSA asset). No stubs,
no "coming soon" (Playbook Amendment I). Wire layout:

  quantizedalert data refresh|health      Layer A
  quantizedalert research <ws>            Layer B (train + backtest via qlib)
  quantizedalert validate <ws> <model>    Layer C (gates + walk-forward + overfit audit)
  quantizedalert deploy <ws> <model>      Layers D+E (registry + schedule)
  quantizedalert daily <ws>               Layers F+G (analysis + alert dispatch)
  quantizedalert alert-test <ws>          Layer G delivery proof (real DSA senders)
  quantizedalert dashboard|serve          Layer H
  quantizedalert discover|experiments|explain   §6 agents (qlib-backed)
  quantizedalert scorecard|economics      §22/§23
  quantizedalert e2e                      full MVP loop, on demand
"""
...
from quantizedalert import __version__
...
logger = logging.getLogger("quantizedalert.cli")
...
def _configure_logging() -> None:
    try:
        ...
        pass
    level = os.environ.get("QUANTIZEDALERT_LOG") or os.environ.get("UNLOCKAID_LOG", "INFO").upper()
...
def _load_ws(pcfg: PlatformConfig, ws: str) -> WorkspaceConfig:
    ...
    if not p.exists():
        ...
        if p2.exists():
            ...
            sys.exit(f"workspace '{ws}' not found: {p} (create with `quantizedalert init-workspace`)")
    return WorkspaceConfig.load(p)


def _alerter(pcfg: PlatformConfig):
    from quantizedalert.alerts.dsa_dispatch import DSAlerter
    dry = os.environ.get("QUANTIZEDALERT_DRY_RUN") or os.environ.get("UNLOCKAID_DRY_RUN") == "1"
...
def cmd_data_refresh(args):
    from quantizedalert.data.health import refresh_dump
    pcfg, store, _ = _ctx()
...
def cmd_alert_test(args):
    ...
    cfg = _load_ws(pcfg, args.workspace)
    alerter = _alerter(pcfg)
    md = (f"### QuantizedAlert delivery test\n\nWorkspace **{cfg.workspace_id}** "
    ...
    print(json.dumps({"engine_source": f"quantizedalert+{alerter.engine_source}",
...
def cmd_serve(args):
    ...
    if args.cron:
        from quantizedalert.alerts.intelligence import AlertIntelligence
        from quantizedalert.analysis.daily import DailyPipeline
        from quantizedalert.deploy.scheduler import DeployScheduler
        sched_store = store
        sch = DeployScheduler(
            sched_store,
    ...
    print(f"QuantizedAlert dashboard on http://{args.host}:{args.port}")
...
def cmd_economics(args):
    pcfg, store, _ = _ctx()
    from quantizedalert.commercial.plans import PLANS, contribution_margin
...
def cmd_discover(args):
    ...
    pcfg, store, engine = _ctx()
    from quantizedalert.agents.research import FactorDiscoveryAgent
    cfg = _load_ws(pcfg, args.workspace)
...
def cmd_experiments(args):
    ...
    from quantizedalert.agents.research import ExperimentAgent
    from quantizedalert.research.runner import ResearchRunner
    cfg = _load_ws(pcfg, args.workspace)
    runner = ResearchRunner(engine, store, pcfg.artifact_dir)
...
def main(argv=None):
    ...
    ap.add_argument("--version", action="version", version=f"quantizedalert {__version__}")
    ...
    s.add_argument("--port", type=int,
                   default=int(os.environ.get("QUANTIZEDALERT_PORT") or os.environ.get("UNLOCKAID_PORT", "8765")))
